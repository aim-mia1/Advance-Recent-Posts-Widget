...
class ADV_Recent_Posts_Widget_Init {
		...
		*/
		...
		public function adv_rpw_get_taxonomies() {
			...
			$post_type_taxonomies = get_object_taxonomies($ptype);
			if(count($post_type_taxonomies)>0)
			{
				foreach ($post_type_taxonomies as $post_type_taxonomy ) {
					...
					foreach($terms as $term)
				   	{
				   		echo '<option value="' . esc_attr($post_type_taxonomy).':'.esc_attr($term->name). '">' . esc_html__($post_type_taxonomy).': '.esc_html__($term->name) . '</option>';
				   	}	
				}
			}
			else
